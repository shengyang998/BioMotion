...
expect_status 'fast rejects zero tests' 1 \
  evaluate_gate fast 633 0 "$GOOD_LOG" "$ZERO"
...
printf '%s\n' '{"totalTestCount":633}' > "$MALFORMED"
expect_status 'missing structured fields fail closed' 1 \
  evaluate_gate fast 633 0 "$GOOD_LOG" "$MALFORMED"
...
expect_output 'fast exact count is reviewed independently of slow' 633 \
