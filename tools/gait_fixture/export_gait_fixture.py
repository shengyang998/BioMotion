...
ASCII file holding exactly the five joints a gait analysis needs (raw MHR root, both
ankles, both toes), in exactly the frame `MHRRetarget.makeBodyFrame(camT: nil)`
produces: metres, Y-up / X-right / Z-toward-camera, source root pinned at the model
...
def build(clip_id: str, tag: str) -> tuple[str, dict]:
    ...
    lines = [
        ...
        "# raw MHR source root pinned at the model constant 0.92398697.",
