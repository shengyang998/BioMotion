...
Use the fail-fast release driver instead of assembling those steps manually:
...
Its default never contacts App Store Connect. The tracked export plist forces a local
`destination=export`; `--validate` explicitly adds remote validation, and only
`--upload` authorizes validation followed by upload of the same private, byte-pinned
snapshot. Manual archive/export does not use `-allowProvisioningUpdates`.

This is a stable-artifact, trusted-account gate, not a sandbox against a malicious
process already running as the same macOS UID. Such a process could race pathname-based
inspection or change the archive after the command returns. Keep the archive quiescent,
run the gate immediately before export, and treat arbitrary same-account code execution
as a compromised release host.
...
```bash
/bin/bash tools/tests/app_resource_boundary_probe_tests.sh
/bin/bash tools/tests/release_dev_model_guard_tests.sh
/bin/bash tools/tests/profile_cms_verifier_tests.sh
/bin/bash tools/tests/testflight_release_gate_tests.sh
```

The resource suite constructs accepted source, arm64 iOS Simulator app, and test-bundle
...
rejection, and validate-before-upload ordering. The successful Apple Distribution path
